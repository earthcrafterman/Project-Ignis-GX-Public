...
function s.filter(c)
	return c:IsType(TYPE_MONSTER) and not c:IsFaceup() and c:IsAttribute(ATTRIBUTE_EARTH)
end
function s.filter2(c)
	return c:IsType(TYPE_MONSTER) and not c:IsRace(RACE_PLANT+RACE_BEAST+RACE_INSECT)
end
