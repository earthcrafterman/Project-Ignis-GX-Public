...
function s.spcon(e,tp,eg,ep,ev,re,r,rp)
	return re:IsActiveType(TYPE_MONSTER) and re:GetHandler():IsSetCard(0x58) and
		re:GetHandler():GetCode()~=id and e:GetHandler():IsPosition(POS_FACEUP_ATTACK)
end
...
function s.spop(e,tp,eg,ep,ev,re,r,rp)
	...
	if #g>0 and c:IsRelateToEffect(e) and c:IsPosition(POS_FACEUP_ATTACK) and c:IsControler(tp) and c:IsRelateToEffect(e) then
