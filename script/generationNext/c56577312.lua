...
s.cfilter=aux.AND(aux.FilterFaceupFunction(Card.IsAttribute,ATTRIBUTE_LIGHT),aux.FilterFaceupFunction(Card.IsRace,RACE_ELEMENTAL))
...
function s.spfilter(c,e,tp)
	return c:IsSetCard(0xe) and c:IsType(TYPE_MONSTER) and c:IsCanBeSpecialSummoned(e,0,tp,false,false)
end
...
function s.spop(e,tp,eg,ep,ev,re,r,rp)
	...
	local tg=Duel.GetMatchingGroup(s.spfilter,tp,LOCATION_HAND,0,nil,e,tp)
	if ct>0 and #tg>0 then
		local sg=aux.SelectUnselectGroup(tg,e,tp,1,ct,aux.dncheck,1,tp,HINTMSG_SPSUMMON)
		if #sg>0 then
