...
function s.initial_effect(c)
	Fusion.AddProcMix(c,true,true,58192742,40240595)
	...
	e3:SetCode(EVENT_SPSUMMON_SUCCESS+EFFECT_CHANGE_LEVEL)
...
s.listed_names={14141448,40240595,CARD_METAMORPHOSIS}
...
function s.ffilter(c,e,tp)
	return c:IsCode(14141448) and c:IsCanBeSpecialSummoned(e,0,tp,false,false) and c:IsType(TYPE_FUSION)
end
function s.fuscon(e,tp,eg,ep,ev,re,r,rp,chk)
	return Duel.IsExistingMatchingCard(s.mfilter,tp,LOCATION_ONFIELD+LOCATION_GRAVE,0,1,nil) and
		Duel.IsExistingMatchingCard(s.ffilter,tp,LOCATION_EXTRA,0,1,nil,e,tp)
end
